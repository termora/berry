...
func (db *Db) UpdateAliases(id int, aliases []string) (err error) {
	...
	if len(aliases) > 0 {
		commandTag, err = db.Pool.Exec(ctx, "update public.terms set aliases = $1, aliases_string = $2, last_modified = (current_timestamp at time zone 'utc') where id = $3", aliases, strings.Join(aliases, ", "), id)
	} else {
		commandTag, err = db.Pool.Exec(ctx, "update public.terms set aliases = array[]::text[], aliases_string = '', last_modified = (current_timestamp at time zone 'utc') where id = $1", id)
	}
	if err != nil {
		return
	}
...
func (db *Db) SetNote(id int, note string) (err error) {
	...
	if len(note) > 0 {
		...
		commandTag, err = db.Pool.Exec(ctx, "update public.terms set note = '', last_modified = (current_timestamp at time zone 'utc') where id = $1", id)
	}
	if err != nil {
		return
	}
