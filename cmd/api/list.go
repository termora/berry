...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Errorf("Error getting terms: %v", err)
...
func (s *Server) listCategory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Errorf("Error getting terms in category %v: %v", id, err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func (s *Server) pronouns(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Errorf("Error getting pronouns: %v", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func (s *Server) tags(w http.ResponseWriter, r *http.Request) {
	tags, err := s.db.Tags()
	if err != nil {
		log.Errorf("Error getting tags: %v", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
