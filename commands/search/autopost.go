...
func (bot *Bot) autopostText(ctx *bcr.Context) (err error) {
	...
	if s, _ := ctx.Flags.GetString("category"); s != "" {
		category, err := bot.DB.CategoryID(s)
		if err != nil {
	...
	var count int
	err = bot.DB.QueryRow(context.Background(), "select count(*) from autopost where guild_id = $1", ctx.Guild.ID.String()).Scan(&count)
	if err != nil {
	...
	if count >= 10 {
		var exists bool
		err = bot.DB.QueryRow(context.Background(), "select exists(select * from autopost where channel_id = $1)", ch.ID).Scan(&exists)
		if err != nil {
			return bot.DB.InternalError(ctx, err)
		}
		if !exists {
	...
	err = bot.setAutopost(ctx.GetGuild().ID, ch.ID, catID, roleID, dur)
	if err != nil {
		return bot.DB.InternalError(ctx, err)
	}
	...
	if roleID != nil && !mentionable {
		s += fmt.Sprintf("\n📝 %v will mention %v for every posted term. Make sure that the bot has the `Mention @everyone, @here, and all roles` permission in %v.", bot.Router.Bot.Username, roleID.Mention(), ch.Mention())
	}
...
func (bot *Bot) autopost(ctx bcr.Contexter) (err error) {
	...
	botPerms, err := ctx.Session().Permissions(ch.ID, bot.Router.Bot.ID)
	if err != nil || !botPerms.Has(discord.PermissionViewChannel|discord.PermissionSendMessages|discord.PermissionEmbedLinks) {
		return ctx.SendfX(":x: %v cannot send term embeds in %v. Please ensure that %v has the *View Channel*, *Send Messages*, and *Embed Links* permissions in that channel.", bot.Router.Bot.Username, ch.Mention(), bot.Router.Bot.Username)
	}
	...
	var count int
	err = bot.DB.QueryRow(context.Background(), "select count(*) from autopost where guild_id = $1", ctx.GetGuild().ID.String()).Scan(&count)
	if err != nil {
		return bot.DB.InternalError(ctx, err)
	}
...
func (bot *Bot) setAutopost(guildID discord.GuildID, channelID discord.ChannelID, categoryID *int, roleID *discord.RoleID, duration time.Duration) (err error) {
	if duration == 0 {
		db.Debug("Removing autopost in %v", channelID)
		_, err = bot.DB.Exec(context.Background(), "delete from autopost where channel_id = $1", channelID)
		return
	}

	db.Debug("Setting autopost in %v", channelID)
	_, err = bot.DB.Exec(context.Background(), `insert into autopost (guild_id, channel_id, category_id, next_post, interval, role_id)
values ($1, $2, $3, $4, $5, $6)
...
func (bot *Bot) setNextTime(channelID discord.ChannelID, time time.Time) (err error) {
	db.Debug("Updating next post time in %v to %s", channelID, time)
	_, err = bot.DB.Exec(context.Background(), "update autopost set next_post = $1 where channel_id = $2", time, channelID)
	return
}
...
func (bot *Bot) autopostLoop() {
	...
	for {
		...
		default:
			...
			if err != nil {
				log.Errorf("Error getting autopost info: %v", err)
			...
			for _, ap := range aps {
				...
				if err != nil {
					log.Errorf("Error running autopost in %v: %v", ap.ChannelID, err)
				}
			}
...
func (bot *Bot) doAutopost(ap Autopost) (err error) {
	sf, _ := discord.ParseSnowflake(ap.GuildID)
	s, _ := bot.Router.StateFromGuildID(discord.GuildID(sf))

	perms, err := s.Permissions(ap.ChannelID, bot.Router.Bot.ID)
	if err != nil {
		bot.setAutopost(0, ap.ChannelID, nil, nil, 0)
		return errors.Wrap(err, "getting permissions")
	}

	if !perms.Has(discord.PermissionViewChannel | discord.PermissionSendMessages) {
		log.Errorf("Can't send messages in %v (guild %v), disabling autopost there.", ap.ChannelID, ap.GuildID)
		return bot.setAutopost(0, ap.ChannelID, nil, nil, 0)
	}

	var t *search.Term
	if ap.CategoryID != nil {
		t, err = bot.DB.RandomTermCategory(*ap.CategoryID, []string{})
	} else {
		t, err = bot.DB.RandomTerm([]string{})
	}
	if err != nil {
	...
	_, err = s.SendMessage(ap.ChannelID, str, bot.DB.TermEmbed(t))
	if err != nil {
		err2 := bot.setAutopost(0, ap.ChannelID, nil, nil, 0)
	...
	err = bot.setNextTime(ap.ChannelID, time.Now().UTC().Add(toAdd))
	if err != nil {
...
func (bot *Bot) autopostList(ctx *bcr.Context) (err error) {
	aps := []Autopost{}
	err = pgxscan.Select(context.Background(), bot.DB, &aps, "select * from autopost where guild_id = $1 order by channel_id", ctx.Message.GuildID.String())
	if err != nil {
		return bot.DB.InternalError(ctx, err)
	}
	...
	for _, ap := range aps {
		...
		if ap.CategoryID != nil {
			if cat := bot.DB.CategoryFromID(*ap.CategoryID); cat != nil {
